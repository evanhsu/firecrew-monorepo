...
export const db: Db = {
    ...
    setBoardState: async (boardId, state) => {
        const newRevisionNumber = await getNextBoardStateRevisionForBoard(
            boardId
        );
    ...
    addTileMoveEvent: async (eventInput) => {
        ...
        if (
            !Object.prototype.hasOwnProperty.call(tileMoveEvents, event.boardId)
        ) {
