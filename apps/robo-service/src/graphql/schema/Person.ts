    ...
    t.field('people', {
      ...
      resolve: async (_root, args, ctx) => {
        ...
        if (args.include?.fuzzyName) {
          matches = await ctx.services.personService.fuzzySearchByName(
        ...
        const personIdsOnBoard = boardState?.rows.map((row) => row.personId);
...
export const PersonQueryIncludeFilterInput = inputObjectType({
  ...
  definition(t) {
    ...
    t.string('fuzzyName', {
      description: "This string will be fuzzy-matched with Peoples' names",
    });
  },
});
...
export const PersonQueryOmitFilterInput = inputObjectType({
  ...
  definition(t) {
    ...
    t.id('boardId', {
      description: 'Omit people who appear on this specific Rotation Board',
    });
  },
});
